...
reps_end = 10 #1 #can be as high as 50
iteration_all = np.arange(reps_st,reps_end)

mu_all = np.round(np.arange(0.01,0.1,0.02),4)
# mu_all = [0.5]
...
lamb_all = np.arange(1000,10000,2000)
...
DataPath_parent = grandparent_directory + '/Data_JOC_R1/Shortest_Path_Tree/'+str(grid[0])+'by'+str(grid[1])+'_grid' +'_depth_'+str(max_depth)+"_"+data_generation_process+"/"
...
for deg in deg_set:
    ...
    for iter in iteration_all:
        ...
        tree = VectorBinaryTree(feat_lb=np.zeros(p),feat_ub=np.ones(p),depth=max_depth_true, max_depth=max_depth, output_dim=d, current_depth=0, feature_names=feat_names)
        # # 2. 打印所有叶子节点的规则和向量值
        # print("*********************************")
        ...
        #         DDR_tree.fit(mu,lamb,dim,x_train,c_train_post,verbose=True,feats_continuous=True); #verbose specifies whether fitting procedure should print progress
