...
class MTP(object):
  ...
  '''
  def fit(self, mu,lamb,dim,X, A, Y, weights=None, feats_continuous=False, verbose=False, refit_leaves=False, seed=None, *leafargs_fit,**leafkwargs_fit):
    ...
    tree_params.mu = mu
    tree_params.lamb_N = lamb/num_obs
    tree_params.dim = dim
    # tree_params.SPO_weight_param = 
    ...
    if leaf_mod.SPO_weight_param == 2.0:
      DDR_loss_Rst = leaf_mod.error(A,Y,mu,lamb/num_obs,dim)
      ...
      leaf_mod_error = fast_avg(leaf_mod.error(A,Y,mu,lamb,dim),weights)
...
##################
        ...
        cur_depth_child_nodes_tem = []
        for n in cur_depth_nodes_inds:
          split_tem = _find_best_split(tree[n], tree_params, leaf_params, verbose, X, A, Y, weights, sum_weights, feats_continuous, Xnames, shuffled_root_inds)
          cur_depth_child_nodes_tem.append(split_tem)
        cur_depth_child_nodes = cur_depth_child_nodes_tem
    ...
    if feats_continuous[j] == True:
      ...
      for k in range(0,num_splits):
        ...
        if (error_l == 1) or (error_r == 1):
          ...
          ### ***** above two lines are replaced with the following codes.###
          if leaf_mod_l.SPO_weight_param == 2.0:
              DDR_loss_Rst = leaf_mod_l.error(A_l,Y_l,tree_params.mu,tree_params.lamb_N,tree_params.dim)
              l_avg_errors[k] = DDR_loss_Rst["obj"]
          else:
              l_avg_errors[k] = np.dot(leaf_mod_l.error(A_l,Y_l,tree_params.mu,tree_params.lamb_N,tree_params.dim),weights_train_l)/sum_weights;
          
          if leaf_mod_r.SPO_weight_param == 2.0:
              DDR_loss_Rst = leaf_mod_r.error(A_r,Y_r,tree_params.mu,tree_params.lamb_N,tree_params.dim)
              r_avg_errors[k] = DDR_loss_Rst["obj"]
          else:
              r_avg_errors[k] = np.dot(leaf_mod_r.error(A_r,Y_r,tree_params.mu,tree_params.lamb_N,tree_params.dim),weights_train_r)/sum_weights;
