...
class LeafModel(object):
  ...
  in the self object.
  '''
  def error(self,A,Y,mu,lamb,dim):
    def get_Edges(dim):
      # dim = 2 #(creates dim * dim grid, where dim = number of vertices)
    ...
    def DDRloss(C,dim,mu,lamb_N):
      ...
      m.setObjective(quicksum([err[k] * err[k] for k in range(len(err))]) + lamb_N*(expr_obj), GRB.MINIMIZE)
      m.optimize()
    ...
    if self.SPO_weight_param == 1.0:
      ...
      return MSE_loss
    elif self.SPO_weight_param == 2.0:
      DDR_loss = DDRloss(Y,dim,mu,lamb)
      self.DDR_RST = DDR_loss
      decision_dict = shortest_path(DDR_loss["sol"])['weights']
      Edge_list,Edge_dict = get_Edges(dim)
      decision_arr = np.asarray([decision_dict[d_key] for d_key in Edge_list])
      self.decision = decision_arr
      return DDR_loss
    else:
      if self.SPO_full_error == True:
        SPO_loss = SPOloss(Y,self.decision) - A
