...
class XG_Processing:
    ...
    def Implement_XG_quad(self,arcs, grid,mis,bump,W_star_all,x_test_all,c_test_all,x_train_all,c_train_all,iteration_all,num_feat,data_generation_process,x_train_quad_all,x_test_quad_all):
        
        cost_XG_Post = {}; cost_XG_Ante = {}; RMSE_in_all = {}; RMSE_out_all = {}
        for iter in iteration_all:
            xg_model = XGBRegressor(n_estimators=2, random_state=42)
            xg_model.fit(x_train_quad_all[iter], c_train_all[iter])
            cost_dem = xg_model.predict(x_test_quad_all[iter])
            cost_in = xg_model.predict(x_train_quad_all[iter])
            RMSE_in_all[iter] = np.sqrt(np.sum((cost_in - c_train_all[iter])**2)/len(cost_in[:,0]))
            RMSE_out_all[iter] = np.sqrt(np.sum((cost_dem - c_test_all[iter])**2)/len(cost_dem[:,0]))
